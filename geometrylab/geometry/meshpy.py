...
class Mesh(object):
    ...
    def __init__(self, file_name=None):
        ...
        self._faces = None  # Maryam
        self._copy = None  # Maryam
        self._mesh_propogate = False # Maryam
    ...
    def copy_mesh(self):
        ...
        copy_mesh._faces = np.copy(self._faces)
    ...
    def make_mesh(self, vertices_list, faces_list):
        ...
        print("make_mesh")
        # vertices_list.append([-1.68,0.384,-0.0159])   # Maryam
        # faces_list.append([0,24,48])
        # print(vertices_list[47])
        # print(vertices_list)
        # print(faces_list)
        ...
        #print(self)  # I think  this was created by me Maryam

    def _make_halfedges(self, vertices_list, faces_list):
        print("_make_halfedges")
        self._V = len(vertices_list)
        self._F = len(faces_list)
        # print(len(faces_list)) # Maryam
        # print(self.F) # Maryam
        # print(len(vertices_list)) # Maryam
        # print(self.V) # Maryam
        self._vertices = np.array(vertices_list, 'f')
        self._faces = np.array(faces_list, 'f')
        orig = []
        face = []
        nexx = []
        prev = []
        ...
        for f in range(self.F):
            ...
            h += N
        # print(orig)  all maryam printing
        # print(face)
        # print(nexx)
        # print(prev)
        # print(twin_i)
        ...
        H[:,2] = nexx
        H[:,3] = prev
        #print(len(twin_i),len(twin_j), len(orig)) # Maryam
    ...
    def vertex_ring_vertices_iterators(self, sort=False, order=False,
        ...
        if order:
            ...
            vj = vj[i]
        if return_lengths:
            i  = np.ones(vj.shape[0], dtype=np.int64)
    ...
    def vertex_normals(self):
        ...
        print("not unit normals: ",  normals)
    ...
    def face_planarity(self, scale_invariant=True):
        ...
        i = np.ones((f.shape[0]),dtype=np.int64)
        j = np.arange(f.shape[0])
        _, k = np.unique(f, True)
    ...
    def orient_faces(self, vertices_list, faces_list):
        F = len(faces_list)
        V = len(vertices_list)
        fmap = -np.ones((V,V), dtype=np.int64)
        inconsistent = np.zeros((V,V), dtype=np.int64)
    ...
    def dual_mesh(self, make_boundary=True):
        ...
        Hb1 = np.zeros((B,6), dtype=np.int64)
        Hb2 = np.zeros((B,6), dtype=np.int64)
        Hb1[:,0] = -1
        Hb2[:,0] = H[b,0]
        Hb1[:,1] = fb
        Hb2[:,1] = HD[H[b,3],1]
