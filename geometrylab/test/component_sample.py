...
sys.path.append('/Users/memo2/Desktop/Summer2023/WebImplementation/geometry-lab-main')
...
class SampleComponent(geo.gui.GeolabComponent):
    '''A sample interactive geolab component'''
    ...
    #                                 Traits
    #--------------------------------------------------------------------------
    '''Define here all component traits, i.e. special attributes that can be
    changed interactively. Traits types are under traits.api. For an
    introduction to traitsUI visit:
    < http://docs.enthought.com/traitsui/index.html >'''
    ...
    #                              Component View
    #--------------------------------------------------------------------------
    '''Here is defined the traits layout in the component panel, through the
    special attribute view. Grouping classes are under traitsui.api. For more
    details visit:
    ...
    def geolab_settings(self):
        '''Function launched when the comonent is added to geolab.
        Here you can:
            - add secondary scenes
            - set the application size'''

        '''This function add a new scene. Up to tree secondary scenes can
        be added. The default scene is 'scene_0'.'''
        self.geolab.add_scene('scene_1')

        '''Here can be set the width and height of the main window, as well
        as additional side windows width.'''
        self.geolab.height = 800
        self.geolab.width = 900
        self.geolab.side_width = 300

    def object_open(self, file_name, geometry):
        '''Function launched after an obj file is read. If the application is
        not running, the function is launched at the start. The file_name and
        the geometry are passed by the main application.'''

        '''Generate a name for the new object and add it to geolab'''
        name = ('mesh_{}').format(self.counter)
        self.geolab.add_object(geometry, name=name)
        self.counter += 1

        '''Get the last added object from geolab'''
        obj = self.geolab.last_object

        '''Check if the geometry is a mesh and initialize its plot functions.
        The initialization plot functions can be defined in the component.
    ...
    def set_state(self, state):
        ...
        Pre-setted states - to do not use - are:
            - 'select_object'
            - 'split_edges':
            - 'collapse_edges':
            - 'move_vertices':
    ...
    def initialize_mesh_plot_functions(self, obj):
        '''A function that initializes all plots available on meshes'''

        '''Add a specific face data plot function to the object faces,
        found in plot setting menu'''
        ...
        '''Set 'something_on_faces' as predefined face plot'''
        obj.face_plot = 'something on faces'

        '''Add a specific vertex data plot function to the object faces,
        found in plot setting menu'''
        ...
        '''Add a edge data plot function to the object edges,
        found in plot setting menu'''
        ...
        '''Set 'something_on_edges' as predefined edge plot'''
        obj.edge_plot = 'something on edges'

    def initialize_points_plot_functions(self, obj):
        '''A function that initializes all plots available on points'''

        '''Add a specific vertex data plot function to the object,
        found in plot setting menu'''
        ...
        '''Set 'something_on_faces' as predefined face plot and update,
        when set, no need of update the plot'''
