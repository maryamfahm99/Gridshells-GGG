...
class GuidedProjectionBase(object):
    ...
    def _initialize_iteration(self):
        ...
        if(self.mesh._mesh_propogate):
            print("if(self.mesh._mesh_propogate)")
            self.initialize_unknowns_vector() # Maryam
            self._reinitialize = False  # this might be risky
    ...
    def _build_constant_matrices(self):
        print("_build_constant_matrices")
        null =  np.zeros([0])
        ...
        #self.build_constant_fairness()    # Maryam commented this line

    def _build_iterative_matrices(self):
        print("_build_iterative_matrices")
        null =  np.zeros([0])
    ...
    def optimize(self):   
        ...
        for i in range(self.iterations):  # 1 iteration
            self._pre_iteration_update()
            self._build_iterative_matrices()
            print("after self._build_iterative_matrices()")
            H = sparse.vstack([self._H0, self._H])
            print("after H")
            ...
            A = (sparse.spmatrix.dot(H.transpose(), H)
                 #+ sparse.spmatrix.dot(K.transpose(), K)\
                      + self._R)
            print("found A")    
            print("PRINT X dim ", X.shape)
            print("Hr hsape: ", H.shape, r.shape)
            a = sparse.spmatrix.dot(H.transpose(), r) \
                        + self.epsilon**2 * X # + sparse.spmatrix.dot(A, X)
                       #+ sparse.spmatrix.dot(K.transpose(), s) \    
            # print("PRINT a ", a)
            # print("PRINT epsilon ", self.epsilon**2 * X)
            # print("PRINT Ax ", sparse.spmatrix.dot(A, X))
            # print("PRINT X ", X)
            # print("PRINT self._R ", self._R)
            try:
                a = a.toarray()
            ...
            X = spsolve(A,a)
            # print("PRINT Solution X ", X)
            iteration += 1
            Xi = np.array(X)
            # print("X differences, ", Xi - X0)
            if self.step != 1:
