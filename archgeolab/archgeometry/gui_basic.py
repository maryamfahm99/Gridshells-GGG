...
from traitsui.api import View, Item, HSplit,ListEditor, \
                         Action, ToolBar, Separator, Controller
...
class GlHandler(Controller):
    ...
    def start_mesh(self): # Hui
        self.info.object.start_mesh()

    def hide_mesh(self): # Hui
        self.info.object.hide_mesh()
...
class GeolabGUI(MultiSceneManager):
    ...
    corner_tolerance_button = Action(action = 'open_corner_tolerance',
                              image = ImageResource(path+'/corners.png'),
                              style = 'push',
    ...
    select_object_button = Action(action = 'select_object',
                           image = ImageResource(path+'/selectobject.png'),
                           style = 'toggle',
                           tooltip = 'Select object',
                           show_label = False)

    flip_edges_button = Action(action = 'flip_edges',
                        image = ImageResource(path+'/flip.png'),
                        style = 'toggle',
    ...
    split_edges_button = Action(action='split_edges',
                         image = ImageResource(path+'/split.png'),
                         style = 'toggle',
                         enabled_when = ('_current_object == \
                                         "Mesh_plot_manager"'),
    ...
    collapse_edges_button = Action(action='collapse_edges',
                            image=ImageResource(path+'/collapse.png'),
                            style='toggle',
                            enabled_when =('_current_object == \
                                           "Mesh_plot_manager"'),
    ...
    catmull_clark_button = Action(action = 'catmull_clark',
                           image = ImageResource(path+'/catmullclark.png'),
                           style = 'push',
                           enabled_when = ('_current_object == \
                                           "Mesh_plot_manager"'),
    ...
    loop_button = Action(action = 'loop',
                  image = ImageResource(path+'/loop.png'),
                  style = 'push',
    ...
    remesh_button = Action(action = 'open_remesh',
                    image = ImageResource(path+'/remesh.png'),
                    style = 'push',
                    enabled_when = ('_current_object == "Mesh_plot_manager"'),
    ...
    move_vertices_button = Action(action = 'move_vertices',
                           image = ImageResource(path+'/movevertex.png'),
                           style = 'toggle',
    ...
    select_vertices_button = Action(action='select_vertices',
                             image=ImageResource(path+'/selectvertices.png'),
                             style='toggle',
    ...
    select_edges_button = Action(action='select_edges',
                          image=ImageResource(path+'/selectedges.png'),
                          style='toggle',
                          enabled_when =('_current_object=="Mesh_plot_manager"'),
    ...
    select_faces_button = Action(action='select_faces',
                          image=ImageResource(path+'/selectfaces.png'),
                          style='toggle',
                          enabled_when =('_current_object == \
                                         "Mesh_plot_manager"'),
    ...
    select_boundary_vertices_button = Action(action='select_boundary_vertices',
                                   image=ImageResource(path+'/boundary.png'),
                                   style='toggle',
                                   enabled_when =('_current_object == \
                                                  "Mesh_plot_manager"'),
    ...
    fix_vertices_button = Action(action='fix_vertices',
                          image=ImageResource(path+'/fixvertices.png'),
                          style='push',
                          tooltip = 'Fix selected vertices',
                          show_label=False)

    unfix_vertices_button = Action(action='unfix_vertices',
                            image=ImageResource(path+'/unfixvertices.png'),
                            style='push',
                            tooltip = 'Unfix selected vertices',
                            show_label=False)

    constrain_vertices_button = Action(action='constrain_vertices',
                                image=ImageResource(path+'/constrain.png'),
                                style='push',
                                tooltip = 'Constrain selected vertices',
                                show_label=False)

    unconstrain_vertices_button = Action(action='constrain_vertices',
                                image=ImageResource(path+'/unconstrain.png'),
                                style='push',
                                tooltip = 'Release selected vertices',
                                show_label=False)

    loads_button = Action(action='open_loads',
                   image=ImageResource(path+'/applyforce.png'),
                   style='push',
                   tooltip = 'loads_tool',
                   show_label=False)

    reset_mesh_button = Action(action='reset_mesh',
                        image=ImageResource(path+'/resetmesh.png'),
                        style='push',
                        tooltip = 'Reset mesh',
                        show_label=False)

    set_reference_button = Action(action='set_reference',
                           image=ImageResource(path+'/setreference.png'),
                           style='push',
                           tooltip = 'Set as reference mesh',
                           show_label=False)

    start_mesh_button = Action(action='start_mesh',  # Hui
                        image=ImageResource(path+'/startmesh.png'),
                        style='push',
                        tooltip = 'Start mesh',
                        show_label=False)
    
    hide_mesh_button = Action(action='hide_mesh',  # Hui
                        image=ImageResource(path+'/hide.png'),
                        style='push',
                        tooltip = 'Hide mesh',
                        show_label=False)

    clear_button = Action(action='clear',  # Hui
                        image=ImageResource(path+'/clear.png'),
                        style='push',
    ...
    toolbar = { 'handler': _handler,
                'resizable' : True,
                'title' : 'ArchGeo', #Hui change
                'icon' : ImageResource(path+'/logo3.png'),
    ...
    def open_obj_file(self, file_name):
        print("open_obj_file")
        mesh = Gridshell()
        print("mesh as gridshell")
        mesh.read_obj_file(file_name)
        self.__geometries.append(mesh)
        print(self.__geometries)
        if not self._closed:
            self.object_open(file_name, mesh)
        else:
            self.__object_open_callbacks.append((file_name, mesh))

    def reinitialize_gridshell(self, vertices_list,faces_list):   # Maryam 
        print("reinitialize_gridshell")
        mesh = Gridshell()
        print("mesh as gridshell")
        mesh.make_mesh(vertices_list, faces_list)
        mesh.initialize()
        self.__geometries.append(mesh) #  I am not sure if I should reset this or not
    ...
    def start_mesh(self,move=False): # Hui
        v0 = self.current_object.geometry.vertices_0
        f0 = self.current_object.geometry.faces_list()
        import numpy as np # Hui
