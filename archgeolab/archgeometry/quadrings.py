...
class MMesh(Mesh):
   ...
   ###--------------------------------------------------------
    ...
    def get_quad_diagonals(self): # Maryam
        ...
        print(len(v),len(va))
        #v1,v2,v3,v4 = self.rr_quadface.T
        # pl1 = make_polyline_from_endpoints(V[va],V[vc])
        pl1 = make_polyline_from_endpoints(V[va],V[v])
        pl2 = make_polyline_from_endpoints(V[v],V[vc])
        pl3 = make_polyline_from_endpoints(V[vb],V[v])
        pl4 = make_polyline_from_endpoints(V[v],V[vd])
        return pl1,pl2,pl3,pl4
    
    def get_normals(self): # Maryam
        V = self.vertices
        v = self.rrv4f4[0]
        N = self.vertex_normals()
        print(len(V), len(N))
        pl = make_polyline_from_endpoints(V[v], V[v]+ N[v])
        return pl
