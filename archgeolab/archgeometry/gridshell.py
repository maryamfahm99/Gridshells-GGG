...
class Gridshell(MMesh):  # Hui
    ...
    def __init__(self, file_name=None):
        ...
        #self._glide_reference_polyline = None # Hui
        ...
        self._handle = np.array([], dtype=np.int64)

        self.__constrained_vertices = np.array([], dtype=np.int64)

        self.__gliding_vertices = np.array([], dtype=np.int64)

        self.__fixed_vertices = np.array([], dtype=np.int64)
    ...
    def handle(self, vertex_index):
        if vertex_index is None:
            ...
            self._handle = np.array(vertex_index, dtype=np.int64)
    ...
    def apply_boundary_load(self, tangent_load, normal_load, vertex_indices):
        F = self.applied_forces
        v = np.array(vertex_indices, dtype=np.int64)
    ...
    def gliding_constraints(self, **kwargs):
        print("gliding_constraints") # Maryam
        N = kwargs.get('N', 3*self.V)
    ...
    def con_avoid_shrinkage_edge_length(self, **kwargs):
        ...
        #c_ai = np.arange(self.E)  #  Maryam
        #min_el = kwargs.get('minLength')
        #minl = min_el**2
        minl = self.min_el**2
    ...
    def constant_constraints(self, **kwargs):
        ...
        print("H for constant constraints",H)
        print(r)
        return H, r

    def iterative_constraints(self, **kwargs):
        print('iterative_constraints')
        N = kwargs.get('N')
        i = j = data = r = np.array([])
        H = sparse.coo_matrix((data,(i,j)), shape=(0,N))
